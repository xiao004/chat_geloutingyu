...
#ifndef ROUTE_PROCESSPOLL_H
#define ROUTE_PROCESSPOLL_H
...
//进程池类
template <typename T>
class route_processpool {
private:
	//使用单例模式, 将构造函数定义为私有的
	// listenfd 为监听 socket
	// 默认创建 4 个子进程
	route_processpool(int listenfd, int process_number = 4);

public:
	//获取实列的函数
	static route_processpool<T>* create(int listenfd, int process_number = 8) {
		if(!m_instance) {
			m_instance = new route_processpool<T>(listenfd, process_number);
		}
...
private:
	...
	// 进程池的静态实列
	static route_processpool<T> *m_instance;
};

// 类外初始化进程池类的静态实列变量
template<typename T>
route_processpool<T>* route_processpool<T>::m_instance = NULL;
...
template <typename T>
route_processpool<T>::route_processpool(int listenfd, int process_number) 
	: m_listenfd(listenfd), m_process_number(process_number), m_idx(-1), m_stop(false) {

		assert((process_number > 0) && (process_number <= MAX_PROCESS_NUMBER));

		// 创建共享内存作为 work server 池
		// 打开创建共享内存文件
		shmfd = shm_open(shm_name, O_CREAT | O_RDWR, 0666);
		assert(shmfd != -1);

		// 重置共享内存文件大小
		int ret = ftruncate(shmfd, MAX_WORK_SERVER * sizeof(work_server_addr));
		assert(ret != -1);
		
		// 共享内存文件地址映射
		share_mem = (work_server_addr*)mmap(NULL, MAX_WORK_SERVER * sizeof(work_server_addr), PROT_READ | PROT_WRITE, MAP_SHARED, shmfd, 0);
		assert(share_mem != MAP_FAILED);

		close(shmfd);

		// 对共享内存中的 work_server_addr 对象初始化
...
template<typename T>
void route_processpool<T>::run_child() {
		...
		for(int i = 0; i < number; i++) {
			...
			if((sockfd == pipefd) && (events[i].events & EPOLLIN)) {
				...
				if(((ret < 0) && (errno != EAGAIN)) || ret == 0) {
					...
					users[connfd].init(m_epollfd, connfd, client_address, share_mem);
...
template <typename T>
void route_processpool<T>::run_parent() {
	setup_sig_pipe();
