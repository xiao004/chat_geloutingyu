...
#include "../internal/semun.h"
#include "../internal/pthread_create_param.h"


// 共享内存文件名
static const char *shm_name = "/my_shm_server";
...
private:
	//使用单例模式, 将构造函数定义为私有的
	// listenfd 为监听 socket
	// connecfd 为发送心跳包的 socket
	// 默认创建 4 个子进程
	work_processpool(int listenfd, int connectfd, struct address route_addr, int process_number = 4);

public:
	//获取实列的函数
	// 监听 socket, 发送心跳包的 socket, 进程池中创建的进程数
	static work_processpool<T>* create(int listenfd, int connectfd, struct address route_addr, int process_number = 4) {
		if(!m_instance) {
			m_instance = new work_processpool<T>(listenfd, connectfd, route_addr, process_number);
		}
...
private:
	...
	// 发送心跳包 & 向 route server 汇报当前机器的用户数，用于 route server 决策 client 路由
...
// 进程池构造函数. 参数 listenfd 是监听 socket, 它必须在创建进程池之前被创建, 否则子进程无法直接引用
// 参数 process_number 指定进程池中子进程的数量
template <typename T>
work_processpool<T>::work_processpool(int listenfd, int connectfd, struct address route_addr, int process_number) 
	: m_listenfd(listenfd), m_connectfd(connectfd), m_route_addr(route_addr), m_process_number(process_number), m_idx(-1), m_stop(false) {
		...
		// 重置共享内存文件大小
		int ret = ftruncate(shmfd, sizeof(int));
		assert(ret != -1);
		
		// 共享内存文件地址映射
		share_mem = (int*)mmap(NULL, sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED, shmfd, 0);
...
// 统一事件源
template <typename T>
void work_processpool<T>::setup_sig_pipe() {
		...
		for(int i = 0; i < number; i++) {
			...
			if((sockfd == pipefd) && (events[i].events & EPOLLIN)) {
				...
				if(((ret < 0) && (errno != EAGAIN)) || ret == 0) {
					...
					users[connfd].init(m_epollfd, connfd, client_address, share_mem, sem_id);
...
void* work_processpool<T>::report_connect(void *data) {
	...
	if(connect(connectfd, (struct sockaddr*)&route_address, sizeof(route_address)) < 0) {
...
void work_processpool<T>::run_parent() {
	...
	ret = pthread_create(&report_connect_pid, NULL, report_connect, (void*)data);
